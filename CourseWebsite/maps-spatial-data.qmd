...
![Azimuthal map centered on Tjärnö such that correct distances to Tjärnö are preserved. Created by https://ns6t.net/azimuth/azimuth.html](images/AzimuthMap.pdf){width="50%"}
...
The code for *SWEREF99* is:
`SWEREF99 <- "+proj=utm +zone=33 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs"`

And for *LAEA* centered on the WGS84 datum:
`LAEA <- "+proj=laea  +lat_0=0 +lon_0=0"`
(to center in Sweden: `laea <- "+proj=laea  +lat_0=16.321998712 +lon_0=62.38583179"`)
...
To start exploring rasters, we are going download information on world topography (altitude). One the most widely used sources of weather data come from  [WorldClim.org](https://worldclim.org/) - these include historical and projected future climage data. They are especially useful for people working on terrestrial species and can be accessed using the `library(geodata)`. 
...
# 
# ggplot() +  
#   geom_raster(data = USA.df, aes(x = x, y = y, fill = USA_elv_msk), show.legend = FALSE) +
#   scale_fill_gradient(low = "lightgray", high = "black") +
#   geom_sf(data = rivers_crop$geometry, col="midnightblue", linewidth = log10(rivers_crop$DIS_AV_CMS+1)/4) +
#   theme_void()
```

This will take a few minutes to load. 30s is the coarser granularity of data. I recommend that you optimise your code and plotting with course resolution rasters and then for the final analyses (and graphics) use the finer resolution. 

___

# Further resources   

## Working with spatial data generally {-}
...
[**sdmpredictors package**](https://www.bio-oracle.org/code.php)  - an essential resource if using marine data that provide access to marine environmental layers in Bio-Oracle (analogous to BioClim)
